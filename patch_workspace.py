...
# ── 1. Update imports to include workspace functions ──
OLD1 = '''import {
...
changes.append(("Update imports", OLD1, NEW1))

# ── 2. Add workspace state after existing data state ──
OLD2 = '''  // ── Leads UI ──
  const [filterStatus, setFilterStatus] = useState("All");'''
NEW2 = '''  // ── Workspaces ──
  const [workspaces, setWorkspaces]           = useState([]);
  const [activeWorkspace, setActiveWorkspace] = useState(null); // workspace obj
  ...
  async function handleJoinWs() {
    ...
    try {
      const ws = await joinWorkspace(joinCode);
...
# ── 3. Add workspace section to dashboard after follow-ups ──
OLD3 = '''    </div>
  );
  // ─── Section: Leads list ──────────────────────────────────────────────────'''
NEW3 = '''      {/* Workspaces */}
      {workspaces.length > 0 && (
        <div style={{ marginTop:28 }}>
          ...
          <div style={{ ...s.card, overflow:"hidden" }}>
            {workspaces.map((ws, i) => {
              const wsLeadCount = 0; // will load on click
              return (
                <div key={ws.id}
                  ...
                  <span style={{ fontSize:10, color:th.text3, background:th.s2, padding:"2px 8px", borderRadius:20, marginRight:4 }}>
                    ⇄ compartido
                  </span>
                  <span style={{ fontSize:12, color:th.text3, fontFamily:"'JetBrains Mono',monospace" }}>
                    {ws.invite_code}
                  </span>
                </div>
              );
  ...
  // ─── Section: Leads list ──────────────────────────────────────────────────'''
changes.append(("Workspace dashboard section", OLD3, NEW3))

# ── 4. Add workspace view to nav items ──
OLD4 = '''  const NAVITEMS = [
...
# ── 5. Add workspace nav section to sidebar ──
...
NEW5 = '''        {/* Workspace nav */}
        {wsNavItems.length > 0 && (
          <div style={{ padding:"8px 10px", borderTop:`1px solid ${th.border}`, marginTop:8 }}>
            <div style={{ fontSize:10, color:th.text3, fontWeight:600, letterSpacing:"0.08em",
              textTransform:"uppercase", padding:"6px 10px 4px" }}>Workspaces</div>
            {wsNavItems.map(item => (
              <button key={item.id}
        ...
        {/* Bottom controls */}
        <div style={{ padding:"16px 14px", borderTop:`1px solid ${th.border}`, display:"flex", flexDirection:"column", gap:8 }}>'''
changes.append(("Workspace sidebar nav", OLD5, NEW5))

# ── 6. Add workspace section view to main content and fix activeView ──
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
            <div>
              <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:4 }}>
                ...
                <span style={{ fontSize:11, color:th.text3, background:th.s2, border:`1px solid ${th.border}`,
                  padding:"2px 10px", borderRadius:20 }}>⇄ workspace compartido</span>
              </div>
              <div style={{ fontSize:12, color:th.text3 }}>
                ...
                <button onClick={()=>navigator.clipboard.writeText(activeWorkspace.invite_code)}
                  style={{ ...s.btnGhost, fontSize:10, padding:"1px 8px", marginLeft:8 }}>Copiar</button>
              </div>
            </div>
            <div style={{ display:"flex", gap:8 }}>
              {selectedIds.size > 0 && (
                <button onClick={()=>setShowAddToWs(true)} style={{ ...s.btnGhost }}>
                  + Agregar {selectedIds.size} lead(s)
                </button>
              )}
            ...
            <div style={{ ...s.card, overflow:"hidden" }}>
              <table style={{ width:"100%", borderCollapse:"collapse" }}>
                ...
                <tbody>
                  {wsLeads.map((l,i)=>(
                    <tr key={l.id}
                      style={{ borderBottom:i<wsLeads.length-1?`1px solid ${th.border}`:"none", cursor:"pointer" }}
...
changes.append(("Workspace view in main", OLD6, NEW6))

# ── 7. Add bulk "Add to workspace" button in leads toolbar ──
...
# ── 8. Add workspace modals before closing modals section ──
OLD8 = '      {ModalAddLead}\n      {ModalAddFU}'
NEW8 = '''      {ModalAddLead}
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:60, padding:16 }}>
          <div style={{ ...s.card, padding:26, width:400, maxWidth:"94vw" }}>
            ...
            <div style={{ marginBottom:14 }}>
              <span style={s.label}>Nombre del workspace</span>
              <input value={newWs.name} onChange={e=>setNewWs(p=>({...p,name:e.target.value}))} style={s.inp} placeholder="Ej: Equipo Ventas California" />
            </div>
            ...
            <div style={{ marginBottom:20 }}>
              ...
              {wsMembers.map(m=>(
                <div key={m.user_id} style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 0", borderBottom:`1px solid ${th.border}` }}>
                  ...
                  <span style={{ flex:1, fontSize:13, color:th.text2, fontFamily:"'JetBrains Mono',monospace", fontSize:11 }}>{m.user_id.slice(0,8)}…</span>
...
# ── 9. Fix activeView to include workspace ──
...
changes.append(("Fix activeView for workspace", OLD9, NEW9))
